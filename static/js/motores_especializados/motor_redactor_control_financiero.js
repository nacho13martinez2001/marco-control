const MotorFinanciero = {
  actualizarFormulario: function () {
    const tipo = document.getElementById("tipoInformeCF").value;
    const bloqueEnajenacion = document.getElementById("formularioCF_Detalle");
    if (tipo === "cesion") {
  ...
  actualizarVisor: function () {
    ...
    if (tipo === "cesion") {
      ...
      const valor = parseFloat(document.getElementById("valor_cf").value || "0");
      const porcentaje = parseFloat(document.getElementById("porcentaje_cf").value || "0");
...
Porcentaje sobre el Presupuesto de Gastos: ${porcentaje.toFixed(2)} %`;
    ...
    document.getElementById("visorCF").innerText = texto;
  },
...
document.addEventListener("DOMContentLoaded", function () {
  const selector = document.getElementById("tipoInformeCF");
  if (selector) {
    selector.addEventListener("change", MotorFinanciero.actualizarFormulario);
    MotorFinanciero.actualizarFormulario(); // ← activa el formulario según el valor inicial
  }
});